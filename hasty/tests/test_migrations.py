...
class TestTags(TestCase):
    def test_haystack_marker_tags_exist(self):
        ...
        for marker in should_exist:
            ...
            assert len(
                t) == 1, f"Marker tag {marker} should exist for Haystack version: {hv.version}"
        ...
        for marker in should_not_exist:
            t = HaystackMarkerTag.objects.filter(tag=marker, version=hv)
            assert len(
                t) == 0, f"Marker tag {marker} should NOT exist for Haystack version: {hv.version}, but does."
    ...
    def test_brick_tags(self):
        ...
        assert len(all_tags) == len(
            bt), f"For Brick version {bvm.version}: {len(all_tags)} tags exist in the Brick ttl file.  Only {len(bt)} tags are populated in the database."
...
class TestHaystackPointTypes(TestCase):
    def test_good_point_types(self):
        ...
        # damper cmd
        point_type = get_hpt_given_marker_tags(
            ['damper', 'cmd', 'point'], hv[0], hpt_annotated)
        assert len(point_type) == 1

        # zone temp sp
        point_type = get_hpt_given_marker_tags(
            ['zone', 'air', 'temp', 'sensor', 'point'], hv[0], hpt_annotated)
        assert len(point_type) == 1

        # occupied sensor point
        point_type = get_hpt_given_marker_tags(
            ['occupied', 'sensor', 'point'], hv[0], hpt_annotated)
        assert len(point_type) == 1

        # zone co2
        point_type = get_hpt_given_marker_tags(
            ['zone', 'air', 'co2', 'sensor', 'point'], hv[0], hpt_annotated)
        assert len(point_type) == 1

        # zone occ heating sp
        point_type = get_hpt_given_marker_tags(
            ['zone', 'air', 'temp', 'occ', 'heating', 'sp', 'point'], hv[0], hpt_annotated)
        assert len(point_type) == 1

        # zone occ cooling sp
        point_type = get_hpt_given_marker_tags(
            ['zone', 'air', 'temp', 'occ', 'cooling', 'sp', 'point'], hv[0], hpt_annotated)
        assert len(point_type) == 1

        # zone unocc heating sp
        point_type = get_hpt_given_marker_tags(
            ['zone', 'air', 'temp', 'unocc', 'heating', 'sp', 'point'], hv[0], hpt_annotated)
        assert len(point_type) == 1

        # zone unocc cooling sp
        point_type = get_hpt_given_marker_tags(
            ['zone', 'air', 'temp', 'unocc', 'cooling', 'sp', 'point'], hv[0], hpt_annotated)
        assert len(point_type) == 1
    ...
    def test_bad_point_types(self):
        ...
        point_type = get_hpt_given_marker_tags(
            ['damper', 'cmd', 'point', 'sensor'], hv[0], hpt_annotated)
        assert len(point_type) == 0

        point_type = get_hpt_given_marker_tags(
            ['discharge', 'air', 'temp', 'sensor', 'point'], hv[0], hpt_annotated)
        assert len(point_type) == 0
